...
"""Weight loading + precompute for the Qwen3.8-Flash-Next Gated DeltaNet layer (adapted from the qwen36 demo loader).
...
# Fidelity-matrix knob (Phase-0, GDN kernel-fidelity plan): QWEN_GDN_WDTYPE=bf16 loads the
...
def remap_flash_next_state_dict(state_dict, config: GDNConfig) -> dict:
    """Map flash-next HF checkpoint keys -> the qwen36 loader namespace.

    Mapping per docs/qwen38-flash-next-gdn-adapt.md section 3 (verified vs
    results/flash-next/module_map.txt lines 74-82). Input is the per-layer
    `linear_attn.` substate (prefix already stripped). The HF checkpoint ships a
    FUSED conv1d.weight (gdn_qkv_dim, 1, k) == (10240, 1, 4) and NO conv bias
    (0 conv1d.bias entries in the module map) — we slice the fused conv into the
    q/k/v split views the loader/precompute code expects, so every downstream
    body stays verbatim. Direct renames: in_proj_qkv.weight -> qkv_proj.weight;
    in_proj_z/in_proj_a/in_proj_b/out_proj/A_log/dt_bias/norm pass through.
    """
    q_dim, k_dim = config.q_dim, config.k_dim
    out = {}
    for name, tensor in state_dict.items():
        if name == "in_proj_qkv.weight":
            out["qkv_proj.weight"] = tensor
        elif name == "conv1d.weight":
            fused = tensor  # [D_total, 1, K] with D_total = q_dim + k_dim + v_dim
            assert fused.shape[0] == q_dim + k_dim + config.v_dim, (
                f"conv1d.weight rows {fused.shape[0]} != q+k+v {q_dim + k_dim + config.v_dim}"
            )
            out["q_conv.weight"] = fused[:q_dim].contiguous()
            out["k_conv.weight"] = fused[q_dim:q_dim + k_dim].contiguous()
            out["v_conv.weight"] = fused[q_dim + k_dim:].contiguous()
            # No conv biases exist in flash-next: q/k/v_conv.bias intentionally absent.
        else:
            out[name] = tensor
    return out
...
def load_gdn_weights(mesh_device, config: GDNConfig, state_dict, tensor_cache_path=None) -> GDNWeights:
    ...
    """
    # Flash-next: remap HF checkpoint keys (incl. fused conv1d split) into the
    # qwen36 loader namespace before any load/precompute runs (see remap docstring).
    state_dict = remap_flash_next_state_dict(state_dict, config)
    ...
    def _precompute_fused_ab_weight():
        ...
        return ttnn.from_torch(fused, dtype=_GDN_PROJ_DTYPE, layout=ttnn.TILE_LAYOUT, device=mesh_device)
