...
class ttMLA:
    ...
    def __init__(
        ...
        # In chunked mode ``seq_len`` is the KV-cache capacity; the fixed activation chunk is
        # supplied separately by TtPrefillBlock. Keep legacy direct ttMLA users functional when
        # they do not supply it by using the established async TP gather for those activations.
        self._has_fixed_active_seq_len = active_seq_len is not None or not is_chunked
        assert (
        ...
        if self.tp_factor > 1 and self._has_fixed_active_seq_len:
            if not self.kv_only:
    ...
    def _q_a_latent(
        ...
        if self.tp_factor > 1:
            ...
            if self._q_a_latent_gather_output is None:
    ...
    def _q_stem(
        ...
        if self.tp_factor == 1 or self._q_a_latent_gather_output is None:
            ttnn.deallocate(qr)
    ...
    def _kv_stem(
        ...
        # All reduce (skip for single-device TP)
        if self.tp_factor > 1:
            if self._kv_stem_gather_output is None:
                ...
                assert seq_len_local == self.active_seq_len_local, (
                    f"KV stem gather was preallocated for {self.active_seq_len_local} local tokens, "
                    f"got {seq_len_local}"
                )
        ...
        if self.tp_factor == 1 or self._kv_stem_gather_output is None:
    ...
    def _sparse_chunked_attn(
        ...
        **_,
        ...
        # wrapper identity here: SP>1 always writes the model-owned persistent scratch, whereas
        # the SP=1 fallback owns a transient selected-slot tensor.
        if self.sp_factor == 1:
    ...
    def _gather_kvpe_prefix(
        ...
        *,
        ...
        Pipeline (all on device): ND→interleaved, then a selected-slot prefix SP all-gather into the
        model-owned worst-case scratch (no-op at sp==1). The cache is already in the op format, so there is
        no read-back dtype/layout conversion. The prefix is rounded to a whole block-cyclic slab; sparse SDPA
        only dereferences current top-k indices, so its unwritten suffix is never consumed."""
        ...
        if self.sp_factor == 1:
            ...
            # behavior, where sparse_sdpa still needs a batch-1 cache.
            ...
            # Block-cyclic storage is meaningful only in complete SP slabs.  The new AG packs this
            # active global prefix at output[:, :, :gathered_dim_size] while retaining the fixed-size
            # output allocation for the next chunk/layer.
