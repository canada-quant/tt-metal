...
// Uniquely named (pcb) so it does not ODR-clash with the sibling factory's kda_pcb:: under unity builds.
namespace kda_pcb {
...
KdaScanWorkDist kda_distribute_scan(
    CoreCoord grid, uint32_t BH, uint32_t Vt, bool vector_gate, bool state_only = false) {
...
uint32_t kda_chunk_prep_cb_size_bytes(
    uint32_t chunk_size,
    uint32_t key_dim,
    uint32_t val_dim,
    bool vector_gate,
...
// PREP
// ---------------------------------------------------------------------------
tt::tt_metal::ProgramDescriptor KdaChunkPrepProgramFactory::create_descriptor(
    const KdaChunkPrepParams& attrs, const KdaChunkPrepInputs& in, std::vector<Tensor>& outputs) {
    ...
    auto dist = kda_factory_detail::distribute_prep(device->compute_with_storage_grid_size(), total_work, core_cap);
    ...
    add_cb(kda_pcb::q, ck, 1, df_io);
    add_cb(kda_pcb::k, ck, 1, df_io);
    add_cb(kda_pcb::v, cv, 1, df_io);
    add_cb(kda_pcb::g, attrs.vector_gate ? ck : Ct, 1, tt::tt_metal::datatype_to_dataformat_converter(in.g.dtype()));
    add_cb(kda_pcb::beta, Ct);
    add_cb(kda_pcb::eye, cc);
    add_cb(kda_pcb::tril, cc);
    add_cb(kda_pcb::ones, cc);
    add_cb(kda_pcb::S, kv, 2);
    add_cb(kda_pcb::decay, attrs.vector_gate ? ck : Ct);
    add_cb(kda_pcb::decay_exp, attrs.vector_gate ? ck : Ct);
    add_cb(kda_pcb::decayfac, attrs.vector_gate ? ck : Ct);
    add_cb(kda_pcb::lmask, cc);
    const auto output_df = [&](uint32_t index) {
        return tt::tt_metal::datatype_to_dataformat_converter(outputs[index].dtype());
    };
    add_cb(kda_pcb::Tinv, cc, 1, output_df(6));
    add_cb(kda_pcb::vbeta, cv, 1, output_df(0));
    add_cb(kda_pcb::kbeta, ck);
    add_cb(kda_pcb::out, cv, 2, df_io);
    add_cb(kda_pcb::u, attrs.vector_gate ? std::max(cv, 3u) : cv);  // KDA startup pacing; Qwen size unchanged
    add_cb(kda_pcb::w, ck, 1, output_df(1));
    add_cb(kda_pcb::qdecay, ck, 1, output_df(2));
    add_cb(kda_pcb::intra, cc, 1, output_df(3));
    add_cb(kda_pcb::s2, kv, 2);
    add_cb(kda_pcb::vnew, attrs.vector_gate ? std::max(cv, Kt) : cv, 1, output_df(5));  // aliased as cb_dl
    add_cb(kda_pcb::ointer, attrs.vector_gate ? std::max(cv, ck) : cv);
    add_cb(kda_pcb::kdec_t, kc, 1, output_df(4));
    add_cb(kda_pcb::supd, kv);
    add_cb(kda_pcb::stmp, kv);
    add_cb(kda_pcb::final_s, kv);
    add_cb(kda_pcb::scr1, scr);

    add_cb(kda_pcb::scr2, scr);
    add_cb(kda_pcb::scr3, scr);
    add_cb(kda_pcb::s3, kv, 2);
    TT_FATAL(
        cb_size_bytes == kda_chunk_prep_cb_size_bytes(
                             attrs.chunk_size,
                             attrs.key_dim,
                             attrs.val_dim,
                             attrs.vector_gate,
    ...
    reader.kernel_source =
        "ttnn/cpp/ttnn/operations/transformer/kda/chunk_preparation/device/kernels/dataflow/"
        "reader_kda_chunk_preparation.cpp";
    ...
    writer.kernel_source =
        "ttnn/cpp/ttnn/operations/transformer/kda/chunk_preparation/device/kernels/dataflow/"
        "writer_kda_chunk_preparation.cpp";
    ...
    compute.kernel_source =
        "ttnn/cpp/ttnn/operations/transformer/kda/chunk_preparation/device/kernels/compute/kda_chunk_preparation.cpp";
    ...
    compute.config = kda_compute_cfg(device->arch(), attrs.compute_kernel_config, attrs.vector_gate);
...
// SCAN
// ---------------------------------------------------------------------------
tt::tt_metal::ProgramDescriptor KdaFinalScanProgramFactory::create_descriptor(
    const KdaFinalScanParams& attrs, const KdaFinalScanInputs& in, std::vector<Tensor>& outputs) {
    const uint32_t BH = attrs.BH;
    const uint32_t NC = attrs.num_chunks;
    const uint32_t Ct = attrs.chunk_size / TILE_HEIGHT;
    const uint32_t Kt = attrs.key_dim / TILE_WIDTH;
    ...
    auto sdist =
        kda_distribute_scan(device->compute_with_storage_grid_size(), BH, Vt_full, attrs.vector_gate, attrs.state_only);
    ...
    if (!attrs.vector_gate) {
        add_cb(kda_pcb::u, cv);
        add_cb(kda_pcb::w, ck);
        add_cb(kda_pcb::qdecay, ck);
        add_cb(kda_pcb::intra, cc);
        add_cb(kda_pcb::kdec_t, kc);
        add_cb(kda_pcb::dl, 1);
        add_cb(kda_pcb::Tinv, cc);
        add_cb(kda_pcb::S, kv);
        add_cb(kda_pcb::s2, kv);
        add_cb(kda_pcb::s3, kv);
        add_cb(kda_pcb::out, cv, 2, df_io);
        add_cb(kda_pcb::final_s, kv);
        add_cb(kda_pcb::vnew, cv);
        add_cb(kda_pcb::ointer, cv);
        add_cb(kda_pcb::supd, kv);
        add_cb(kda_pcb::stmp, kv);
        add_cb(kda_pcb::scr1, scr);
    } else {
        add_cb(kda_pcb::u, cv, 1, input_df(in.v_beta));
        add_cb(kda_pcb::w, ck, 1, input_df(in.kd));
        add_cb(kda_pcb::kdec_t, kc, 1, input_df(in.k_dec_t));
        add_cb(kda_pcb::dl, Kt, 1, input_df(in.dl));
        add_cb(kda_pcb::Tinv, cc, 1, input_df(in.t_inv));
        add_cb(kda_pcb::S, kv);
        add_cb(kda_pcb::s2, kv);
        add_cb(kda_pcb::s3, kv);
        add_cb(kda_pcb::final_s, kv);
        ...
        if (attrs.summary_pair) {
            add_cb(kda_pcb::qdecay, kv);
            add_cb(kda_pcb::intra, kv);
            add_cb(kda_pcb::ointer, kv);
            add_cb(kda_pcb::out, kv, 1, df_io);
        } else {
            add_cb(kda_pcb::qdecay, ck, 1, input_df(in.q_decay));
            add_cb(kda_pcb::intra, cc, 1, input_df(in.intra));
            add_cb(kda_pcb::ointer, cv);
            add_cb(kda_pcb::out, cv, 2, df_io);
        }
    }
    ...
    KernelDescriptor reader;
    reader.kernel_source =
        "ttnn/cpp/ttnn/operations/transformer/kda/final_scan/device/kernels/dataflow/reader_kda_final_scan.cpp";
    reader.source_type = KernelDescriptor::SourceType::FILE_PATH;
    reader.core_ranges = cores;
    reader.compile_time_args = reader_ct;
    reader.config = ReaderConfigDescriptor{};
    ...
    KernelDescriptor writer;
    writer.kernel_source =
        "ttnn/cpp/ttnn/operations/transformer/kda/final_scan/device/kernels/dataflow/writer_kda_final_scan.cpp";
    writer.source_type = KernelDescriptor::SourceType::FILE_PATH;
    writer.core_ranges = cores;
    writer.compile_time_args = writer_ct;
    writer.config = WriterConfigDescriptor{};
    ...
    KernelDescriptor compute;
    compute.kernel_source =
        "ttnn/cpp/ttnn/operations/transformer/kda/final_scan/device/kernels/compute/kda_final_scan.cpp";
    compute.source_type = KernelDescriptor::SourceType::FILE_PATH;
    compute.core_ranges = cores;
...
tt::tt_metal::ProgramDescriptor KdaAffinePrefixProgramFactory::create_descriptor(
    ...
    dataflow.kernel_source =
        "ttnn/cpp/ttnn/operations/transformer/kda/affine_prefix/device/kernels/dataflow/"
        "reader_writer_kda_affine_prefix.cpp";
    ...
    KernelDescriptor compute;
    compute.kernel_source =
        "ttnn/cpp/ttnn/operations/transformer/kda/affine_prefix/device/kernels/compute/kda_affine_prefix.cpp";
    compute.source_type = KernelDescriptor::SourceType::FILE_PATH;
    compute.core_ranges = cores;
    compute.compile_time_args = {Kt, Vt, G, attrs.compose_only};
    compute.config = kda_compute_cfg(device->arch(), attrs.compute_kernel_config);
