...
struct KdaChunkPrepParams {
    ...
    // Bit i stores prep output i as BF16; unsupported combinations are rejected by validation.
...
struct KdaChunkPrepProgramFactory {
    static tt::tt_metal::ProgramDescriptor create_descriptor(
        const KdaChunkPrepParams&, const KdaChunkPrepInputs&, std::vector<Tensor>&);
};

struct KdaChunkPrepOperation {
    using operation_attributes_t = KdaChunkPrepParams;
    using tensor_args_t = KdaChunkPrepInputs;
    using spec_return_value_t = std::vector<tt::tt_metal::TensorSpec>;
    using tensor_return_value_t = std::vector<Tensor>;
    using program_factory_t = std::variant<KdaChunkPrepProgramFactory>;
...
struct KdaFinalScanProgramFactory {
    static tt::tt_metal::ProgramDescriptor create_descriptor(
        const KdaFinalScanParams&, const KdaFinalScanInputs&, std::vector<Tensor>&);
};

struct KdaFinalScanOperation {
    using operation_attributes_t = KdaFinalScanParams;
    using tensor_args_t = KdaFinalScanInputs;
    using spec_return_value_t = std::vector<tt::tt_metal::TensorSpec>;
    using tensor_return_value_t = std::vector<Tensor>;
    using program_factory_t = std::variant<KdaFinalScanProgramFactory>;
