...
template <uint32_t output_chunks_per_page, uint32_t output_chunk_size, uint32_t num_devices, uint32_t slice_step>
...
public:
    ...
    FORCE_INLINE std::pair<uint32_t, uint32_t> next() {
        if constexpr (slice_step > 1) {
            ...
            const uint32_t stripe_group = local_chunk / output_chunks_per_stripe_;
            const uint32_t chunk_in_stripe = local_chunk % output_chunks_per_stripe_;
            ++sent_;
            return {stripe_group * stripe_distance_chunks_ + stripe_ * output_chunks_per_stripe_ + chunk_in_stripe, 0};
        }
...
private:
    void send_queued_chunks() {
        static_assert(!coalesce_contiguous_pages, "bank-owned writes bypass the scatter accumulator");
        noc.async_writes_flushed();
