...
struct HighBwAllGatherParams {
    ...
    // output buffer. `gathered_dim_size` is the global (post-gather) valid extent along dim; the output
    // allocation remains the worst-case full extent. Their values are hash-excluded and patch only
    // indexed kernel runtime arguments, so slots/prefixes reuse one cached program.
